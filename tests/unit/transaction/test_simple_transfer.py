...
class TestSimpleTransfer(unittest.TestCase):
    ...
    def test_simple_transfer_transaction_creation(self):
        ...
        self.assertEqual(len(tx.funds), 1)
        self.assertEqual(tx.funds[self.sender.public_key][self.receiver.public_key]["USDC"], 20)
...
class TestSimpleTransferPytest:
    ...
    def test_transfer_creation_with_fixtures(self, test_account):
        ...
        assert len(tx.funds) == 1
        assert tx.funds[sender.public_key][recipient.public_key]["USDC"] == 20
